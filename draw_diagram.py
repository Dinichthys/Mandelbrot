...
file_names = ['data/plot_Naive_clang++_O3.png',
              'data/plot_Array_clang++_O3.png',
...
with open('Output_1_clang_2.txt', 'r') as f:
   data = [float(line.strip()) for line in f]

version_num = 0
plt.hist(data [250 : 751], bins = 40, color = 'blue', edgecolor = 'black', range= (3.92*10**8, 3.98*10**8))
plt.grid(axis='y', linestyle='--')
plt.title("Наивная версия в компиляторе clang++ с флагом -O3")
...
with open('Output_2_clang_2.txt', 'r') as f:
   data = [float(line.strip()) for line in f]

version_num = 1
plt.hist(data [250 : 751], bins = 40, color = 'blue', edgecolor = 'black', range= (1.02*10**8, 1.06*10**8))
plt.grid(axis='y', linestyle='--')
plt.title("Реализация на массивах в компиляторе clang++ с флагом -O3")
plt.xlabel('Задержка (такты)')
plt.ylabel('Количество повторений')
plt.savefig(file_names [version_num], dpi = 300)
plt.clf ()
...
version_num = 2
plt.hist(data [250 : 751], bins = 40, color = 'blue', edgecolor = 'black', range= (6.5*10**7, 6.56*10**7))
plt.grid(axis='y', linestyle='--')
plt.title("Реализация с интринсиками размера 256 в компиляторе clang++ с флагом -O3")
plt.xlabel('Задержка (такты)')
plt.ylabel('Количество повторений')
plt.savefig(file_names [version_num], dpi = 300)
plt.clf ()
...
version_num = 3
plt.hist(data [250 : 751], bins = 40, color = 'blue', edgecolor = 'black', range= (4.06*10**7, 4.11*10**7))
plt.grid(axis='y', linestyle='--')
...
plt.savefig("clang++_O3.png", dpi = 300)
